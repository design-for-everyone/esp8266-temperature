...
const mqttClient = mqtt.connect('mqtt://192.168.0.178:1883');

mqttClient.on('connect', () => {
    console.log('Server is connected to MQTT');
   
    // subscribing to the fake sensor
    mqttClient.subscribe('weather/temperature', (error, granted) => {
        if (error) {
    ...
    mqttClient.subscribe('weather/humidity', (error, granted) => {
        if (error) {
            console.log('An error occured');
            console.log(error);
        } else {
